...
impl NodeMapBuilder {
    fn build<T: Node + 'static>(tree: &Pin<Box<T>>) -> HashMap<DynNodeId, NodeInfo> {
        let mut hashmap = HashMap::with_capacity(1000);
        let tree = tree.as_ref().get_ref();
        // Topmost node has no parent.
        // SAFETY: root tree is pinned
        hashmap.insert(
            unsafe { tree.node_id() },
            NodeInfo {
                parent: None,
...
impl NodeVisitor for NodeMapBuilder {
    fn visit_with_parent<P: DynNode + 'static, C: DynNode + 'static>(
        ...
        // SAFETY: we only visit if the root tree has been pinned already.
        self.0.insert(
            unsafe { child.node_id() },
            NodeInfo {
                parent: Some(unsafe { parent.node_id() }),
                children: vec![],
            },
        );
        if let Some(info) = self.0.get_mut(&unsafe { parent.node_id() }) {
            info.children.push(unsafe { child.node_id() });
        }
    }
}
...
impl<T: IntoView> Tree<T> {
        ...
        F: FnOnce() -> T + 'static,
    {
        ...
        let node_info = NodeMapBuilder::build(&view);
...
impl<T: Node> Tree<T> {
    pub fn root_node(&self) -> DynNodeId {
        // SAFETY: After self has been built, the entire tree is pinned, so it is safe to get the
        // node_id.
        unsafe { self.inner.node_id() }
    }
    ...
    pub fn relayout_nodes(&mut self) -> Option<()> {
        ...
        for node in nodes.clone() {
            tracing::info!("node: {:?}", unsafe { node.as_ref() }.get_style());
            tracing::info!("before: {:?}", self.layout_tree.get_layout(node));
            let mut current = Some(node);
        ...
        for node in nodes {
            tracing::info!("after: {:?}", self.layout_tree.get_layout(node));
        }
        Some(())
    }
        ...
        self.owner.clone().with(|| {
            self.manager.clone().with(|| {
                ...
                for node in self.render_order.render_order() {
                    let layout = self
                        .layout_tree
                        .get_layout(node.clone())
                        .unwrap()
                        .abs_layout;
                    let mut node = node.clone();
                    unsafe { node.as_mut() }.render(&mut mesh, systems, &layout);
                }
                mesh
            })
        })
...
macro_rules! define_view {
    ($name:ident => $body:expr) => {
        mod $name {
            use super::*;
            pub type Type = impl IntoView;
            #[define_opaque(app_body::Type)]
            pub fn make() -> Type {
                $body
            }
        }
