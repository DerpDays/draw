...
impl ScrollGeometry {
    fn from_layout(layout: &Layout, current_scroll: f32) -> Self {
        let scrollbar_w = layout.scrollbar_size.width;
        let viewport_h = layout.size.height - layout.border.top - layout.border.bottom;
        // Taffy's content_size usually represents the bounding box of children.
        // We add the current scroll to reconstruct the full virtual height.
        let content_h = layout.content_size.height + current_scroll;
...
pub fn scroll_area(scroll_amount: Signal<f32>) -> ElementBuilder<ScrollArea> {
    ...
    ElementBuilder::new(ScrollArea { scroll_amount }).on_mouse(move |node, ctx| {
        let layout = node.get_abs_layout();
        let geo = ScrollGeometry::from_layout(layout, scroll_amount.get_untracked());
        ...
        match ctx.payload().kind {
            MouseEventKind::Press {
                    ...
                    && local_mouse_y <= (current_bar_y + geo.bar_height)
                {
                    ...
                    // Jump scroll
                    let new_px = geo
                        .scroll_to_px((local_mouse_y - (geo.bar_height / 2.0)) / geo.track_height);
