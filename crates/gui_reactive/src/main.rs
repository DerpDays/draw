...
use taffy::{
    Size, Style,
    prelude::{TaffyMaxContent, length, percent},
};
...
async fn main() {
    ...
    let mut tree = Tree::build(Size::MAX_CONTENT, mgr, || {
        ...
        div().style(StyleWrapper::default()).child((
            text(texta)
                .style(Style {
                    size: length(100.),
                    ..Style::DEFAULT
        ...
        .with(|| async {
            tree.owner
                ...
                .with(|| async {
                    ...
                    tracing::info!(
                        "source value before: {:?}",
                        tree.get(*tree.get(tree.root_node()).children().get(0).unwrap())
                            .inner_as_any()
                            .downcast_ref::<gui_reactive::widgets::primitives::Text>()
                            .unwrap()
                            .text
                            .get_untracked()
                    );
