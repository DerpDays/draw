...
impl<C: ApplyCoordinates> Text<C> {
    ...
    fn render_glyph(
        ...
        if let Some(mask_glyph) = mask_atlas.is_allocated(cache_key.clone()) {
            let area: Box2D<f32> = match mask_glyph.data {
                TextureData::Text(data) => Box2D::from_origin_and_size(
                    Point2D::new(
                        glyph_x as i32 + data.placement_left,
                        glyph_y as i32 - data.placement_top,
                    )
                    .cast(),
                    Size2D::new(data.width, data.height).cast(),
                ),
                _ => unreachable!(
                    "mask glyph can only have associated data of type TextureData::Text"
                ),
            };
            ...
            self.atlas_keys.mask_glyphs.push(mask_glyph);
            return Some(());
        }
        if let Some(color_glyph) = color_atlas.is_allocated(cache_key.clone()) {
            let area: Box2D<f32> = match color_glyph.data {
                TextureData::Text(data) => Box2D::from_origin_and_size(
                    Point2D::new(
                        glyph_x as i32 + data.placement_left,
                        glyph_y as i32 - data.placement_top,
            ...
            self.atlas_keys.color_glyphs.push(color_glyph);
            return Some(());
        }
