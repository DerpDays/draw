...
use crate::cli::{Arguments, Command, IpcCommand, Message};
...
fn create_daemon<T: AsRef<Path>>(args: Arguments, sock_path: T) -> Result<()> {
    ...
    let mut conn = {
        ...
        future::block_on(executor.run(crate::wayland::WaylandConnection::new()))
    }
    ...
    if outputs.is_empty() {
        ...
        for (output, info) in conn.wayland().get_displays() {
            if let Some(name) = &info.name && outputs.contains(name) {
                    // conn.add_canvas()
            }
        }
                        ...
                        match message {
                            ...
                            Message::SetInteractive => {
                                for view in state.canvas_outputs.values_mut() {
                                    if let Err(err) = view.set_mode(&mut state.shareable, wayland::OverlayMode::Interactive) {
                            ...
                            Message::SetVisible => {
                                for view in state.canvas_outputs.values_mut() {
                                    if let Err(err) = view.set_mode(&mut state.shareable, wayland::OverlayMode::Visible) {
                            ...
                            Message::SetHidden => {
                                for view in state.canvas_outputs.values_mut() {
                                    if let Err(err) = view.set_mode(&mut state.shareable, wayland::OverlayMode::Hidden) {
